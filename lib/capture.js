...
function isValidFarsiHook(hookText) {
  if (!hookText || !hookText.trim()) return false;
  const cleaned = hookText.trim().toLowerCase();

  // Common safety / error / disclaimer keywords
  const invalidKeywords = ["safety", "user safety", "error", "sorry", "policy", "cannot fulfill", "refuse", "disclaimer", "language model", "ai model", "free tier", "quota"];
  if (invalidKeywords.some(kw => cleaned.includes(kw))) {
    return false;
  }

  // Count Persian/Arabic and English characters
  const persianCharCount = (hookText.match(/[\u0600-\u06FF]/g) || []).length;
  const englishCharCount = (hookText.match(/[a-zA-Z]/g) || []).length;

  // Must contain Persian characters and not be dominated by English
  if (persianCharCount === 0 || englishCharCount > persianCharCount) {
    return false;
  }
...
async function generateHookText(text, opts = {}) {
  const openrouterApiKey = opts.openrouterApiKey || process.env.OPENROUTER_API_KEY;
  
  // 1. Try OpenRouter first if key exists
  if (openrouterApiKey) {
    try {
      console.log("Generating Instagram hook using OpenRouter (openrouter/free)...");
      const promptText = `تو یک کپیرایتر حرفه‌ای و متخصص ایجاد تعامل (Engagement) و وایرال کردن ویدیو در اینستاگرام هستی. من متن یک توییت سیاسی/اجتماعی را به تو می‌دهم. وظیفه تو این است که یک «هوک» (Hook) بسیار کوتاه، چالش‌برانگیز و پرکشش بسازی که در ثانیه اول ویدیو مخاطب را میخکوب کند و او را ناخودآگاه وادار به واکنش و نوشتن کامنت کند. \n\nقوانین سختگیرانه ساخت هوک:\n۱. هوک باید به گونه‌ای باشد که مخاطب یا شدیداً مخالف آن شود، یا احساس کند باید حقیقت یا نظر خودش را زیر ویدیو بنویسد.\n۲. به هیچ وجه درخواست مستقیم برای کامنت گذاشتن یا نظر دادن نکن (از جملاتی مثل «نظر شما چیه؟» یا «کامنت بذارید» استفاده نکن)، بلکه خودِ جمله باید ذهن مخاطب را قلقلک دهد تا مجبور به نوشتن شود.\n۳. طول هوک بسیار کوتاه باشد (نهایتاً ۳ تا ۷ کلمه) تا برای نمایش کلمه به کلمه روی ویدیو کاملاً ایده‌آل باشد.\n۴. زبان خروجی فقط و فقط فارسی باشد.\n۵. بسیار مهم: خروجی تو باید فقط و فقط خودِ متن هوک باشد. هیچگونه مقدمه، توضیحات اضافه، علامت نقل‌قول یا کلمه‌ی اضافه‌ای (مثل «هوک:») در پاسخ قرار نده.\n\nمتن توییت:\n${text}`;

      let responseData = null;

      try {
        const openrouter = new OpenRouter({ apiKey: openrouterApiKey });
        const sdkRes = await openrouter.chat.send({
          chatRequest: {
      ...
      if (rawContent && rawContent.trim()) {
        const hookText = rawContent.trim().replace(/^["']|["']$/g, '');
        if (isValidFarsiHook(hookText)) {
          console.log(`Generated hook using OpenRouter: "${hookText}"`);
          return hookText;
        } else {
          console.warn(`OpenRouter returned invalid/English/safety text ("${hookText}"). Falling back to Gemini...`);
        }
      }
    } catch (err) {
      console.warn("Failed to generate hook using OpenRouter:", err.message, "Falling back to Gemini...");
    }
  }

  // 2. Fallback to Gemini if OpenRouter failed, key missing, or returned non-Persian/safety response
  ...
  if (geminiApiKey) {
    ...
    for (const model of models) {
      try {
        console.log(`Generating 3-second Instagram hook using Gemini (${model})...`);
        const response = await ai.models.generateContent({
          model,
          contents: text,
          config: {
            systemInstruction: "از این متن ورودی یک هوک متنی به فقط به زبان فارسی خیلی جذاب با معنی و پر کشش برای اول یک ویدو کوتاه اینستاگرامی بساز\nکوتاه، مستقیم و پرکشش\nخروجی فقط متن هوک باشد بدون هیچ توضیح اضافه"
          }
        });

        const hookText = response.text || response.candidates?.[0]?.content?.parts?.[0]?.text;
        if (hookText && hookText.trim()) {
          const cleanedText = hookText.trim().replace(/^["']|["']$/g, '');
          if (isValidFarsiHook(cleanedText)) {
            console.log(`Generated hook using Gemini (${model}): "${cleanedText}"`);
            return cleanedText;
          }
        }
      } catch (err) {
        console.warn(`Failed to generate hook from Gemini (${model}):`, err.message);
      }
    }
  }

  // 3. Fallback: use first 8 words of the tweet if all AI models failed
  console.log("All hook generation models failed. Using fallback text.");
...
function generateTextAnimationHtml(text, opts) {
  ...
  if (opts.bgColor) {
    bgStyle = `background: ${opts.bgColor};`;
  ...
  <style>
    * {
    ...
    .text-container {
      ...
      ${opts.textBgColor ? `background-color: ${opts.textBgColor}; padding: 30px 40px; border-radius: 20px; box-shadow: 0 10px 40px rgba(0,0,0,0.3);` : ''}
    }
    .word {
...
async function renderTextAnimation(opts) {
  ...
  try {
    ...
    for (let i = 0; i < totalFrames; i++) {
      ...
      const framePath = join(tmpDir, `xshot-anim-frame-${String(i).padStart(4, '0')}.jpg`);
    ...
    if (opts.complex) {
      ...
      if (audioFile) {
        compileCmd = `ffmpeg -y -framerate 15 -i "${tmpDir}/xshot-anim-frame-%04d.jpg" -i "${audioFile}" -vf "setpts=${complexConfig.setpts},eq=${complexConfig.eq}" -r 30 -c:v libx264 -preset ultrafast -pix_fmt yuv420p -c:a aac -b:a 192k "${opts.output}"`;
      } else {
        compileCmd = `ffmpeg -y -framerate 15 -i "${tmpDir}/xshot-anim-frame-%04d.jpg" -vf "setpts=${complexConfig.setpts},eq=${complexConfig.eq}" -r 30 -c:v libx264 -preset ultrafast -pix_fmt yuv420p "${opts.output}"`;
      }
    } else {
      if (audioFile) {
        compileCmd = `ffmpeg -y -framerate 15 -i "${tmpDir}/xshot-anim-frame-%04d.jpg" -i "${audioFile}" -r 30 -c:v libx264 -preset ultrafast -pix_fmt yuv420p -c:a aac -b:a 192k "${opts.output}"`;
      } else {
        compileCmd = `ffmpeg -y -framerate 15 -i "${tmpDir}/xshot-anim-frame-%04d.jpg" -r 30 -c:v libx264 -preset ultrafast -pix_fmt yuv420p "${opts.output}"`;
      }
    }
  ...
  try {
    ...
    if (opts.complete && tweetFrame) {
      ...
      try {
        ...
        const hasImage = await tweetFrame.evaluate(() => {
          ...
          const mediaContainer = document.querySelector('div[class*="media"]') || document.querySelector('div._3rK4h');
        ...
        if (hasMedia) {
          console.log("Tweet has media (video/photo). Hiding tweet text to avoid duplication with the intro.");
          await tweetFrame.evaluate(() => {
            const textEl = document.querySelector('[data-testid="tweetText"]');
            if (textEl) textEl.style.display = "none";
          });
        } else {
          console.log("Tweet has no media (text-only). Keeping original tweet text visible.");
        }
      } catch (err) {
      ...
      if (tweetText) {
        ...
        try {
          introText = await generateHookText(tweetText, opts);
        } catch (err) {
        ...
        const introOpts = {
          ...
          output: introOutPath,
          complex: false,
          textColor: opts.hookTextColor || opts.textColor,
          bgColor: opts.hookBgColor || opts.bgColor,
          textBgColor: opts.hookTextBgColor || opts.textBgColor,
        };
