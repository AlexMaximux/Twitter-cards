...
async function generateHookText(text, opts = {}) {
  const apiKey = opts.openrouterApiKey || process.env.OPENROUTER_API_KEY;
  if (!apiKey) {
    console.log("OPENROUTER_API_KEY environment variable is not set. Skipping hook generation, using fallback text.");
  ...
  try {
    ...
    console.warn("Failed to generate hook using OpenRouter:", err.message);
  }

  // Fallback: use first 8 words of the tweet if hook generation failed
  console.log("Hook generation failed. Using fallback text.");
  const words = text.trim().split(/\s+/);
  if (words.length > 8) {
    return words.slice(0, 8).join(" ") + "...";
  }
