  ...
  const contents = [
    {
      ...
      parts: [
        {
          text: `از این متن یک هوک ۳ ثانیه برای انتشار ویدو توی اینستاگرام برام بنویس خروجی فقط متن هوک باشد بدون هیچ توضیح اضافه 
...
async function appendEndVideo(mainVideo, endVideo, opts, tmpDir) {
  ...
  const endWithAudio = await ensureAudioTrack(endVideo, tmpDir);
  const fixedMain = await ensureAudioTrack(mainVideo, tmpDir);

  const fixedEndVideo = join(tmpDir, `end-fixed-${Date.now()}.mp4`);
  // Conform end video to match width, height, framerate, and audio layout
  const conformCmd = `ffmpeg -y -i "${endWithAudio}" -filter_complex "[0:v]scale=${opts.width}:${opts.height}:force_original_aspect_ratio=decrease,pad=${opts.width}:${opts.height}:(ow-iw)/2:(oh-ih)/2,setsar=1[v];[0:a]aresample=48000,aformat=channel_layouts=stereo[a]" -map "[v]" -map "[a]" -r 30 -c:v libx264 -preset ultrafast -pix_fmt yuv420p -c:a aac -b:a 192k "${fixedEndVideo}"`;
  await execPromise(conformCmd);
  ...
  try {
    if (endWithAudio !== endVideo) unlinkSync(endWithAudio);
    if (fixedMain !== mainVideo) unlinkSync(fixedMain);
    unlinkSync(fixedEndVideo);
...
export async function captureScreenshot(opts) {
  ...
  try {
    ...
    if (opts.complete && !hasVideo) {
      console.warn("Warning: Complete mode (-complete) is only supported for tweets that contain a video. Ignoring complete mode.");
      opts.complete = false;
    }
    ...
    if (hasVideo) {
      ...
      if (useComplex) {
        ...
        if (existsSync(configPath)) {
          ...
          console.log("complex.json not found, using default complex parameters:", complexConfig);
        }
