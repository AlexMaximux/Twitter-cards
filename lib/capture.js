...
async function generateHookText(text) {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) {
    console.log("GEMINI_API_KEY environment variable is not set. Skipping hook generation, using fallback text.");
  ...
  for (const model of models) {
    try {
      console.log(`Generating 3-second Instagram hook using ${model}...`);
      ...
      if (hookText && hookText.trim()) {
        resultText = hookText.trim().replace(/^["']|["']$/g, '');
        console.log(`Generated hook using ${model}: "${resultText}"`);
        break;
      }
    } catch (err) {
      console.warn(`Failed to generate hook from ${model}:`, err.message);
    }
  }
  ...
  // Fallback: use first 8 words of the tweet if all models failed
  console.log("Both hook generation models failed. Using fallback text.");
  const words = text.trim().split(/\s+/);
  if (words.length > 8) {
    return words.slice(0, 8).join(" ") + "...";
  }
